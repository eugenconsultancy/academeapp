// frontend/src/components/geoservice/NearbyClasses.jsx
...
export function NearbyClasses() {
  ...
  // Fetch nearby classes when location is available
  useEffect(() => {
    if (location) {
      fetchNearbyClasses();
    }
  ...
  const fetchNearbyClasses = async () => {
    ...
    try {
      ...
      const detail = err.response?.data?.detail || 'Failed to fetch nearby classes';
      setError(detail);
    } finally {
      setLoading(false);
    }
  ...
  // ── Directions via backend (source of truth) ─────────────────
  const openDirections = async (classItem) => {
    if (!location) {
    ...
    try {
      ...
      if (result?.maps_url) {
        ...
        toast.error('Could not obtain directions URL from the server.');
      }
    } catch (err) {
      const msg =
        err.response?.data?.detail || 'Failed to get directions. Please try again.';
      toast.error(msg);
    }
  };

  // Distance colour helper (kept for visual consistency)
  ...
  if (geoError) {
    return (
      <div style={styles.errorBox}>
        ...
        <button onClick={getLocation} style={styles.retryBtn}>
          Retry
        </button>
      </div>
    );
  ...
  if (!location) {
    return (
      <div style={styles.emptyBox}>
        ...
        <button onClick={getLocation} style={styles.enableBtn}>
          Enable Location
        </button>
      </div>
    );
  ...
  if (error) {
    return (
      <div style={styles.errorBox}>
        <FiAlertCircle size={16} />
        <span>{error}</span>
        <button onClick={fetchNearbyClasses} style={styles.retryBtn}>
    ...
    <div>
      ...
      <div style={styles.list}>
        {classes.map((cls, i) => (
          <div
            key={cls.entry_id}
            style={{ ...styles.card, animationDelay: `${i * 50}ms` }}
          >
            <div style={styles.cardRow}>
              ...
              <div style={styles.cardDist}>
                <span
                  style={{
                    ...styles.distVal,
                    color: getDistanceColor(cls.distance_meters),
                  }}
                >
                  {cls.distance_display}
                </span>
                <span style={styles.distTime}>
                  🚶 {cls.walking_time_minutes} min
                </span>
              </div>
            </div>
