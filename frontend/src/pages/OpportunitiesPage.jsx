...
export default function OpportunitiesPage() {
  ...
  const filteredOpportunities = search.trim()
    ? (opportunities || []).filter(o =>
      ...
      stripHtml(o.description).toLowerCase().includes(search.toLowerCase())
    )
    : opportunities;
  ...
  return (
    <>
      <style>{`
        ...
        .opp-modal { position: relative; z-index: 61; width: 100%; max-width: 700px; background: rgba(255,255,255,0.96); border: 1px solid rgba(255,255,255,0.6); border-radius: 24px; backdrop-filter: blur(28px); box-shadow: 0 24px 64px rgba(0,0,0,0.18); animation: oppIn .22s cubic-bezier(0.16,1,0.3,1) both; overflow: hidden; }
        ...
        .opp-modal-body { padding: 20px 22px; display: flex; flex-direction: column; gap: 12px; max-height: 70vh; overflow-y: auto; }
        ...
        .ql-container { min-height: 180px; font-family: 'Outfit', sans-serif; }
      ...
      <div className="opp-root">
        <div className="opp-header">
          ...
          <div className="opp-header-btns">
            <button onClick={() => refetch()} className="opp-btn opp-btn-refresh"><FiRefreshCw size={14} /> Refresh</button>
            {isAdmin && (
              <button onClick={() => setShowCreateModal(true)} className="opp-btn opp-btn-create"><FiPlus size={15} /> Post Opportunity</button>
            )}
          </div>
        </div>

        <div className="opp-search-bar">
          <div className="opp-search-wrap"><FiSearch size={15} className="opp-search-icon" /><input type="text" placeholder="Search opportunities..." value={search} onChange={e => setSearch(e.target.value)} /></div>
        ...
        <div className="opp-categories">
          ...
          {Object.entries(categoryConfig).map(([key, { label, icon: Icon }]) => (
            <button key={key} onClick={() => setCategory(key)} className={`opp-cat-btn ${category === key ? 'active' : ''}`}><Icon size={13} />{label}</button>
          ))}
        </div>
                  ...
                  <div className="opp-body">
                    <div className="opp-top-row">
                      <div className="opp-badges">
                        <span className={`opp-badge ${config.bg} ${config.text}`}><config.icon size={11} />{config.label}</span>
                    ...
                    <Link to={`/opportunities/${opp.id}`} className="opp-title-link"><h3>{opp.title}</h3></Link>
                    <div className="opp-desc" dangerouslySetInnerHTML={{ __html: opp.description.length > 200 ? opp.description.substring(0, 200) + '...' : opp.description }} />
                    <div className="opp-bottom-row">
                      <div className="opp-meta-left">
                        ...
                        {opp.posted_by?.full_name && <span className="opp-meta-item"><FiUser size={11} />{opp.posted_by.full_name}</span>}
                      ...
                      {opp.link && <a href={opp.link} target="_blank" rel="noopener noreferrer" className="opp-link-btn"><FiExternalLink size={13} /> Apply</a>}
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="opp-modal-overlay">
          ...
          <div className="opp-modal">
            ...
            <div className="opp-modal-body">
              ...
              <div>
                <label className="opp-label">Description (rich text)</label>
                <ReactQuill
                  theme="snow"
              ...
              <div><label className="opp-label">Expires in (days)</label><input className="opp-input" type="number" min={1} max={365} value={createForm.expires_in_days} onChange={e => setCreateForm({ ...createForm, expires_in_days: parseInt(e.target.value) || 120 })} /></div>
            </div>
            <div className="opp-modal-footer">
              <button className="opp-btn-cancel" onClick={() => setShowCreateModal(false)}>Cancel</button>
              <button className="opp-btn-submit" style={{ background: 'linear-gradient(135deg, #10b981, #059669)' }} onClick={() => createMutation.mutate(createForm)} disabled={!createForm.title.trim() || !createForm.description.trim() || createMutation.isPending}>
