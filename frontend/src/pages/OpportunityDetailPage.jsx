...
// ── Safe number parser for API counters ──────────────────────
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 py-8 px-4">
            <div className="max-w-3xl mx-auto">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 p-6 md:p-8">
                    {/* Category badge */}
                    <span
                        className={`inline-block px-3 py-1 rounded-full text-xs font-semibold mb-4 ${categoryColors[opportunity?.category] || categoryColors.other
                            }`}
                    >
                        {opportunity?.category
                            ? String(opportunity.category).charAt(0).toUpperCase() +
                            String(opportunity.category).slice(1)
                            : 'General'}
                    </span>
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500 dark:text-gray-400 mb-6 pb-6 border-b border-gray-100 dark:border-gray-700">
                        {opportunity.posted_by && (
                            <span className="flex items-center gap-1">
                                <FiUser className="w-4 h-4" /> {opportunity.posted_by.full_name}
                            </span>
                        )}
                        <span className="flex items-center gap-1">
                            <FiClock className="w-4 h-4" /> {formatPostedDate(opportunity.created_at)}
                        </span>
                    </div>
                    ...
                    {opportunity.link && (
                        <a
                            ...
                            className="inline-flex items-center gap-2 px-6 py-3 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-xl shadow-md hover:shadow-lg transition-all mb-6"
                        >
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full p-6">
                        ...
                        <select
                            ...
                            className="w-full px-4 py-2 border rounded-xl mb-3 dark:bg-gray-700 dark:border-gray-600"
                        >
                        ...
                        <textarea
                            ...
                            rows={3}
                            className="w-full px-4 py-2 border rounded-xl resize-none dark:bg-gray-700 dark:border-gray-600"
                            placeholder="Details (optional)"
                        />
                        <div className="flex gap-3 mt-4">
                            <button
                                onClick={() => setShowReportModal(false)}
                                className="flex-1 py-2 rounded-lg text-sm hover:bg-gray-100 dark:hover:bg-gray-700"
                            >
                                Cancel
                            </button>
                            <button
