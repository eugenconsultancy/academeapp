...
function AttendanceCTA({ nextMarkableClass, location, onSuccess, isDark }) {
  ...
  const urgencyMap = {
    normal: { border: '#f3601c', glow: 'rgba(238,62,9,.18)', btn: 'linear-gradient(135deg,#4f46e5,#7c3aed)' },
    warning: { border: '#d97706', glow: 'rgba(245,158,11,.22)', btn: 'linear-gradient(135deg,#d97706,#f59e0b)' },
    critical: { border: '#ef4444', glow: 'rgba(239,68,68,.25)', btn: 'linear-gradient(135deg,#dc2626,#ef4444)' },
  };
  ...
  return (
    <div className="hp-cta-wrap" style={{
      '--cta-border': u.border, '--cta-glow': u.glow,
      background: cardBg,
      border: `1.5px solid ${u.border}`,
      boxShadow: `0 0 0 4px ${u.glow}, 0 2px 12px rgba(0,0,0,.06)`,
    }}>
      ...
      <div style={{ flex: 1, minWidth: 0, paddingLeft: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 4 }}>
          ...
          <span style={{ fontSize: '.8rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.08em', color: u.border }}>
            Check-in window open
          </span>
        </div>
        <p style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: '1rem', fontWeight: 700, color: textMain, letterSpacing: '-.015em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: timeLeft ? 3 : 0 }}>
...
function NeedsAttentionSection({ items, isDark }) {
  ...
  const bg = isDark ? 'rgba(55,40,8,0.45)' : '#fffbf0';
  const border = isDark ? '1px solid rgba(245,158,11,.2)' : '1px solid rgba(245,158,11,.18)';
  ...
  const Row = ({ item }) => (
    <Link to={item.to || '#'} className="hp-na-row" style={{ color: 'inherit', textDecoration: 'none' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {item.is_urgent && (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: '.58rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.06em', color: textUrgent, background: 'rgba(245,158,11,.14)', padding: '2px 8px', borderRadius: 99, marginBottom: 4 }}>
  ...
  return (
    <div className="hp-attention-section" style={{ background: bg, border, borderLeft: '4px solid #f59e0b', borderRadius: 16, padding: '20px 20px 14px', boxShadow: '0 2px 16px rgba(245,158,11,.07)' }}>
      {critical.length > 0 && (
        <div style={{ marginBottom: normal.length ? 16 : 0 }}>
          <p style={{ fontSize: '.6rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.1em', color: textUrgent, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#f59e0b', display: 'inline-block', animation: 'hpCtaDot 1.3s ease-in-out infinite' }} />
            Critical
          </p>
      ...
      {normal.length > 0 && (
        <div>
          {critical.length > 0 && <p style={{ fontSize: '.6rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.1em', color: isDark ? '#9ca3af' : '#6b7280', marginBottom: 10, marginTop: 4 }}>Informational</p>}
          {normal.map(item => <Row key={item.key} item={item} />)}
        </div>
      )}
      <Link to="/announcements" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, padding: '10px 0 2px', fontSize: '.72rem', fontWeight: 700, color: '#6366f1', textDecoration: 'none', borderTop: `1px solid ${isDark ? 'rgba(99,102,241,.15)' : 'rgba(99,102,241,.08)'}`, marginTop: 12 }}>
...
function StatCard({ label, value, sublabel, icon: Icon, color, accentBg, trend, total, isDark }) {
  ...
  const borderColor = isDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,.06)';
  const textMain = isDark ? '#f1f5f9' : '#111827';
  const labelColor = isDark ? '#94a3b8' : '#9ca3af';
  const subColor = isDark ? '#64748b' : '#6b7280';

  return (
    <div className="hp-stat-card" style={{ borderRadius: 16, padding: '18px 16px 16px', background: bg, border: `1px solid ${borderColor}`, boxShadow: '0 1px 4px rgba(0,0,0,.04)' }}>
...
function ClassRow({ c, highlight, isDark }) {
  const bgHighlight = highlight ? (isDark ? 'rgba(99,102,241,0.09)' : 'rgba(99,102,241,.05)') : 'transparent';
  const textMain = isDark ? '#e2e8f0' : '#1f2937';
  const textMuted = isDark ? '#94a3b8' : '#9ca3af';
  const pillStyle = (type) => {
    if (type === 'green') return { background: 'rgba(16,185,129,.15)', color: isDark ? '#6ee7b7' : '#059669' };
    if (type === 'amber') return { background: 'rgba(245,158,11,.15)', color: isDark ? '#fbbf24' : '#d97706' };
  ...
  return (
    <div style={{
      ...
      borderBottom: highlight ? 'none' : `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,.04)'}`,
    }}>
      <span style={{ fontSize: '.58rem', fontWeight: 800, padding: '4px 8px', borderRadius: 8, background: highlight ? 'rgba(99,102,241,.15)' : (isDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,.05)'), color: highlight ? '#6366f1' : textMuted, whiteSpace: 'nowrap', flexShrink: 0, marginTop: 2 }}>
...
/* ══════════════════════════════════════════════════════════════
   FEATURE SECTIONS — with updated copy per brief
══════════════════════════════════════════════════════════════ */
const FEATURE_SECTIONS = [
  {
    ...
    emoji: '💼',
    title: 'Opportunities',
    tagline: 'Your launchpad for scholarships, internships, and leadership.',
    intro: 'Opportunities is your centralised hub for scholarships, grants, internships, and leadership programmes tailored to students like you. Discover curated listings — filtered by category, deadline, and eligibility — so the right opportunity always reaches you first. Apply directly, track your submissions, and receive timely reminders so no deadline slips through.',
    bullets: [
  ...
  },
  {
    ...
    intro: 'Announcements keeps every student informed, whether it is urgent administrative notices, timetable changes, or upcoming events. Urgent items surface instantly at the top of your feed with push notifications delivered directly to your device. A moderated student-posting workflow filters out noise, so every notice you see is accurate and relevant.',
    bullets: [
      { icon: FiAlertCircle, text: 'Urgent notices from administration surfaced instantly.' },
      { icon: FiMessageSquare, text: 'Student posts go through an approval workflow to prevent spam.' },
  ...
  },
  {
    ...
    intro: 'The Lost & Found feature makes recovering lost belongings quicker and safer. Browse a real-time photo feed of items turned in across campus, each tagged with the location and time it was found. A verified claim process confirms identity before reuniting items, protecting both finder and owner at every step.',
    bullets: [
      { icon: FiSearch, text: 'Photo-based listings with location tags across campus.' },
      { icon: FiUsers, text: 'Verified claim process with identity confirmation to prevent fraud.' },
      { icon: FiActivity, text: 'Real-time feed of newly posted items — check before giving up.' },
    ],
    stat: '85%',
  ...
  },
  {
    ...
    intro: 'Classes gives you a live view of your timetable and makes marking attendance effortless. Geo-verification ensures you are physically at the venue before a check-in is accepted, keeping records accurate and fair. Detailed semester analytics highlight attendance trends per unit, so you can address gaps before they affect your standing.',
    bullets: [
  ...
  {
    id: 'blog',
    gradient: 'linear-gradient(135deg, rgba(99,102,241,.07) 0%, rgba(139,92,246,.04) 100%)',
    border: 'rgba(99,102,241,.15)',
    accent: '#8b5cf6',
    accentBg: 'rgba(99,102,241,.1)',
    icon: FiBookOpen,
    emoji: '✍️',
    title: 'Blogs & Student Insights',
    tagline: 'Explore educational insights and student experiences beyond your campus.',
    intro: 'The Blog brings together perspectives, discoveries, and lessons from students across disciplines. Read articles covering university life, academic success strategies, career guidance, emerging technologies, scholarships, entrepreneurship, mental wellness, and insights into institutions around the world. Every post is a window into a different corner of student life — written by those who live it.',
    bullets: [
      { icon: FiEdit3, text: 'Articles on university life, wellness, career paths & global institutions.' },
      { icon: FiLayers, text: 'Filter by topic — tech, academics, entrepreneurship, scholarships, and more.' },
      { icon: FiAward, text: 'Top-rated posts curated weekly so the best content always finds you.' },
    ],
    stat: '500+',
    statLabel: 'Student articles published',
    to: '/blog',
    cta: 'Read the Blog',
    reverse: false,
  },
  {
    ...
    emoji: '🗺️',
    title: 'Campus Map',
    tagline: 'Navigate campus smarter and spend less time searching for locations.',
    intro: 'Campus Map gives you a precise, interactive guide to every corner of your institution. Find lecture halls, laboratories, administrative offices, hostels, libraries, cafeterias, and student services with ease. Live indoor positioning guides you step-by-step to any destination, while venue details — capacity, accessibility, opening hours, and contacts — ensure you always arrive prepared.',
    bullets: [
...
function FeatureSection({ section, index, isDark }) {
  ...
  const borderColor = isDark ? 'rgba(255,255,255,.08)' : section.border;
  const accentBg = isDark ? `${section.accent}20` : section.accentBg;
  ...
  return (
    <section
      ...
      style={{
        ...
        transition: `opacity .7s ${index * .08}s cubic-bezier(.16,1,.3,1), transform .7s ${index * .08}s cubic-bezier(.16,1,.3,1)`,
      }}
    >
      {/* Text side */}
      <div style={{ order: reverse ? 2 : 1 }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '5px 14px', borderRadius: 99, background: accentBg, border: `1px solid ${borderColor}`, marginBottom: 20 }}>
          ...
          <span style={{ fontSize: '.62rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.1em', color: section.accent }}>{section.title}</span>
        </div>

        <h2 style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: 'clamp(1.5rem,3vw,2.2rem)', fontWeight: 800, letterSpacing: '-.04em', color: textMain, lineHeight: 1.15, marginBottom: 14 }}>
          {section.tagline}
        </h2>

        {/* Structured intro: What / Why / How */}
        <p style={{ fontSize: '.88rem', color: bodyColor, lineHeight: 1.75, marginBottom: 28, maxWidth: 460 }}>
          {section.intro}
        </p>
        ...
        <ul style={{ listStyle: 'none', margin: '0 0 32px', padding: 0, display: 'flex', flexDirection: 'column', gap: 14 }}>
          {section.bullets.map((b, i) => (
            <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 13 }}>
              <div style={{ width: 30, height: 30, borderRadius: 9, background: accentBg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: section.accent, flexShrink: 0, marginTop: 2 }}>
                <b.icon size={14} />
              </div>
              <span style={{ fontSize: '.86rem', fontWeight: 500, color: bodyColor, lineHeight: 1.6 }}>{b.text}</span>
            </li>
          ))}
        </ul>

        <Link to={section.to} style={{
          display: 'inline-flex', alignItems: 'center', gap: 8,
          padding: '12px 24px', borderRadius: 13,
          background: `linear-gradient(135deg,${section.accent},${section.accent}cc)`,
          color: '#fff', fontFamily: "'DM Sans',sans-serif", fontSize: '.84rem', fontWeight: 700,
          textDecoration: 'none', boxShadow: `0 4px 16px ${accentBg}`,
      ...
      <div style={{ order: reverse ? 1 : 2 }}>
        <div style={{
          borderRadius: 24,
          background: isDark ? `${section.accent}12` : section.gradient,
          border: `1px solid ${borderColor}`,
          padding: '40px 36px', display: 'flex', flexDirection: 'column', gap: 22,
          boxShadow: isDark ? '0 8px 32px rgba(0,0,0,.35)' : `0 8px 32px rgba(0,0,0,.05),0 0 0 1px ${section.border}`,
          position: 'relative', overflow: 'hidden',
        }}>
          <div style={{ position: 'absolute', right: 22, top: 18, fontSize: '4.5rem', opacity: .06, userSelect: 'none', lineHeight: 1 }}>
            {section.emoji}
          </div>
          <div>
            <div style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: 'clamp(2.4rem,5vw,3.6rem)', fontWeight: 800, letterSpacing: '-.06em', color: textMain, lineHeight: 1 }}>
              {section.stat}
            </div>
            <div style={{ fontSize: '.74rem', fontWeight: 600, color: textMuted, marginTop: 6 }}>{section.statLabel}</div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
            {section.bullets.map((b, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,.75)', borderRadius: 12, border: `1px solid ${isDark ? 'rgba(255,255,255,.08)' : 'rgba(255,255,255,.85)'}` }}>
                <b.icon size={14} style={{ color: section.accent, flexShrink: 0 }} />
                <span style={{ fontSize: '.74rem', fontWeight: 600, color: bodyColor }}>{b.text.split(' — ')[0]}</span>
              </div>
            ))}
          </div>
        </div>
...
function CampusIntelligence({ weather, isDark }) {
  ...
  const slides = [
    ...
    weather && { icon: weather.icon, label: 'Weather', text: weatherInsight(weather.desc) || `${weather.temp}°C, ${weather.desc}. Humidity ${weather.humidity}%, wind ${weather.wind} m/s.` },
  ...
  const border = isDark ? '1px solid rgba(255,255,255,.07)' : '1px solid rgba(0,0,0,.06)';
  ...
  return (
    <div style={{ borderRadius: 16, background: cardBg, border, borderLeft: `3px solid ${dotColor}`, padding: '16px 18px', boxShadow: '0 1px 4px rgba(0,0,0,.04)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '.63rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.1em', color: '#4f46e5' }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {['‹', '›'].map((ch, j) => (
            <button key={ch} onClick={() => setIdx(i => j === 0 ? (i - 1 + slides.length) % slides.length : (i + 1) % slides.length)}
              style={{ width: 26, height: 26, border: `1px solid ${isDark ? 'rgba(255,255,255,.1)' : 'rgba(0,0,0,.08)'}`, borderRadius: 7, background: 'transparent', cursor: 'pointer', fontSize: '.9rem', color: '#6b7280', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
        ...
        <div>
          <p style={{ fontSize: '.62rem', fontWeight: 700, color: isDark ? '#94a3b8' : '#9ca3af', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 4 }}>{slide.label}</p>
          <p style={{ fontSize: '.82rem', fontWeight: 500, color: textMain, lineHeight: 1.6 }}>{slide.text}</p>
        </div>
      </div>
      <div style={{ display: 'flex', gap: 5, marginTop: 14, alignItems: 'center' }}>
        {slides.map((_, i) => (
          <button key={i} onClick={() => setIdx(i)} style={{ width: i === idx ? 18 : 6, height: 6, borderRadius: 99, background: i === idx ? dotColor : isDark ? 'rgba(255,255,255,.1)' : 'rgba(0,0,0,.1)', border: 'none', cursor: 'pointer', padding: 0, transition: 'all .25s' }} />
        ))}
      </div>
    </div>
  );
...
function ActivityFeed({ todayClasses, announcements, opportunities, claimedItems, isDark }) {
  ...
  const textMuted = isDark ? '#94a3b8' : '#9ca3af';
  const borderColor = isDark ? 'rgba(255,255,255,.04)' : 'rgba(0,0,0,.04)';

  if (!items.length) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '24px 16px', color: textMuted }}>
      <span style={{ fontSize: '1.4rem', marginBottom: 8, opacity: .5 }}>📭</span>
...
function SectionTitle({ children, to, linkLabel, tier = 'secondary', isDark }) {
  const dotColors = { primary: '#4f46e5', secondary: '#6366f1', tertiary: '#9ca3af', passive: '#d1d5db' };
  const textColor = isDark ? '#f1f5f9' : '#111827';
  const dot = dotColors[tier];
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '32px 0 14px' }}>
      <h2 style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: tier === 'primary' ? '1.15rem' : '.95rem', fontWeight: 700, letterSpacing: '-.03em', color: textColor, margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ width: 3, height: tier === 'primary' ? 18 : 14, borderRadius: 99, background: dot, flexShrink: 0 }} />
        {children}
      </h2>
      {to && (
        <Link to={to} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '.7rem', fontWeight: 700, color: '#6366f1', textDecoration: 'none', padding: '4px 10px', borderRadius: 8, transition: 'color .15s,background .15s' }}
          onMouseEnter={e => { e.currentTarget.style.color = '#4f46e5'; e.currentTarget.style.background = isDark ? 'rgba(99,102,241,.1)' : 'rgba(99,102,241,.07)'; }}
...
function Card({ title, dotColor, to, linkLabel, children, elevated = false, accent = false, isDark }) {
  const bg = isDark ? 'rgba(15,23,42,0.85)' : '#ffffff';
  const border = isDark ? `1px solid ${accent ? 'rgba(99,102,241,.15)' : 'rgba(255,255,255,.06)'}` : `1px solid ${accent ? 'rgba(99,102,241,.12)' : 'rgba(0,0,0,.06)'}`;
    ...
    <div style={{ background: bg, border, borderRadius: 16, padding: 18, boxShadow: elevated ? (isDark ? '0 4px 20px rgba(0,0,0,.4)' : '0 4px 20px rgba(0,0,0,.07)') : '0 1px 4px rgba(0,0,0,.04)' }}>
      {(title || to) && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          ...
          {to && (
            <Link to={to} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '.68rem', fontWeight: 700, color: '#6b7280', textDecoration: 'none', padding: '3px 9px', borderRadius: 7, background: isDark ? 'rgba(255,255,255,.06)' : 'rgba(0,0,0,.04)' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(100px,1fr))', gap: 10 }}>
      {links.map(({ to, label, icon: Icon, color = '#6366f1', bg = 'rgba(99,102,241,.08)' }) => (
        <Link key={to} to={to} className="hp-ws-link" style={{
          '--ws-color': color, '--ws-bg': bg,
          background: isDark ? 'rgba(15,23,42,0.85)' : '#ffffff',
          borderColor: isDark ? 'rgba(255,255,255,.07)' : 'rgba(0,0,0,.07)',
          color: isDark ? '#d1d5db' : '#374151',
        }}>
          <div className="hp-ws-icon-wrap" style={{ background: isDark ? 'rgba(255,255,255,.05)' : 'rgba(0,0,0,.04)' }}><Icon size={18} /></div>
...
function PlatformIntro({ isDark }) {
  ...
  return (
    <div ref={ref} style={{
      ...
      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '5px 16px', borderRadius: 99, background: isDark ? 'rgba(99,102,241,.12)' : 'rgba(99,102,241,.08)', border: `1px solid ${isDark ? 'rgba(99,102,241,.22)' : 'rgba(99,102,241,.15)'}`, marginBottom: 20 }}>
        <FiZap size={13} style={{ color: '#6366f1' }} />
        <span style={{ fontSize: '.63rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.1em', color: '#6366f1' }}>Everything in one place</span>
      </div>
      <h2 style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: 'clamp(1.8rem,4vw,3rem)', fontWeight: 800, letterSpacing: '-.04em', color: textMain, lineHeight: 1.15, maxWidth: 600, margin: '0 auto 16px' }}>
        Your campus, intelligently connected
      </h2>
      <p style={{ fontSize: '.93rem', fontWeight: 400, color: textMuted, lineHeight: 1.75, maxWidth: 520, margin: '0 auto 52px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap' }}>
        {[
          ...
          <div key={i} style={{ padding: '20px 36px', borderRight: i < 3 ? `1px solid ${isDark ? 'rgba(255,255,255,.07)' : 'rgba(0,0,0,.07)'}` : 'none' }}>
...
export default function HomePage() {
  ...
  const workspaceLinks = [
    ...
    { to: '/blog', label: 'Blog', icon: FiBookOpen, color: '#ec4899', bg: 'rgba(236,72,153,.1)' },
    ...(isLeader ? [
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after { box-sizing:border-box; }
        ...
        .hp-root {
          ...
          color:inherit; position:relative; z-index:10;
        }
        @media(max-width:640px){ .hp-root{padding:20px 16px 100px} }

        /* ── Hero ── */
        .hp-hero {
          border-radius:24px; padding:48px 48px 44px;
          background:rgba(9,13,28,.93);
          backdrop-filter:blur(20px) saturate(1.4);
          border:1px solid rgba(255,255,255,.08);
          box-shadow:0 0 0 1px rgba(99,102,241,.16), 0 24px 80px rgba(0,0,0,.38), inset 0 1px 0 rgba(255,255,255,.06);
          margin-bottom:20px; color:#fff; position:relative; overflow:hidden;
        }
        .dark .hp-hero { background:rgba(4,6,16,.94); }
        .hp-hero::before {
          content:''; position:absolute; inset:0; pointer-events:none;
          background:
            radial-gradient(ellipse at 15% 30%,rgba(99,102,241,.24) 0%,transparent 55%),
            radial-gradient(ellipse at 88% 75%,rgba(139,92,246,.18) 0%,transparent 50%);
        }
        .hp-hero::after {
          content:''; position:absolute; inset:0; pointer-events:none;
          background-image:url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none'%3E%3Cg fill='rgba(255,255,255,0.018)'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E");
        }
        ...
        .hp-hero-tag {
          ...
          color:#a5b4fc; margin-bottom:16px; backdrop-filter:blur(8px);
        }
        .hp-hero-name {
          font-family:'Bricolage Grotesque',sans-serif;
          font-size:clamp(1.9rem,5vw,3.2rem); font-weight:800; letter-spacing:-.045em;
          line-height:1.08; margin:0 0 8px; color:#fff;
        }
        .hp-hero-name span { color:#d3e203; }
        .hp-hero-sub { font-size:.87rem; font-weight:300; color:rgba(180,220,230,.88); margin:0 0 24px; max-width:380px; }
        .hp-hero-pills { display:flex; flex-wrap:wrap; gap:8px; }
        .hp-hero-pill {
        ...
        .hp-hero-meta-pill {
          display:flex; align-items:center; gap:5px; padding:6px 13px; border-radius:10px;
          background:rgba(255,255,255,.07); border:1px solid rgba(255,255,255,.08);
          font-size:.68rem; font-weight:600; color:rgba(255,255,255,.85); white-space:nowrap;
        }
        ...
        .hp-na-row {
          ...
          margin:0 -6px; transition:background .15s;
        }
        ...
        .hp-stat-card { transition:transform .2s,box-shadow .2s; }
        ...
        .hp-ws-link:hover {
          ...
          color:var(--ws-color); transform:translateY(-4px);
          box-shadow:0 8px 20px rgba(0,0,0,.08);
        }
        .dark .hp-ws-link:hover { box-shadow:0 8px 20px rgba(0,0,0,.6); }
        ...
        .dark .hp-divider { border-top-color:rgba(255,255,255,.07); }
      `}</style>

      <div style={{ position: 'fixed', inset: 0, zIndex: 0, background: isDark ? '#060810' : '#F8F7FF' }} />
      ...
      <div className="hp-root">
        ...
        <div className="hp-hero">
          <div className="hp-hero-inner">
            ...
            <div className="hp-hero-right">
              ...
              <div style={{ width: isMobile ? 80 : 100, height: isMobile ? 80 : 100, borderRadius: '50%', background: isDark ? 'rgba(99,102,241,.15)' : 'rgba(99,102,241,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 2 }}>
                <span style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: '1.5rem', fontWeight: 800, color: '#a5b4fc' }}>{attendedCount}/{totalClasses}</span>
                <span style={{ fontSize: '.56rem', fontWeight: 700, color: 'rgba(165,180,252,.7)', textTransform: 'uppercase', letterSpacing: '.06em' }}>Attended</span>
              </div>
        ...
        {!loadingCls && allMarked && (
          <div className="hp-done-banner" style={{ background: isDark ? 'rgba(16,185,129,.12)' : undefined, color: isDark ? '#6ee7b7' : undefined, borderColor: isDark ? 'rgba(16,185,129,.22)' : undefined }}>
              ...
              : todayClasses?.length
                ...
                : (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '28px 16px', color: isDark ? '#94a3b8' : '#9ca3af', gap: 10 }}>
                    <span style={{ fontSize: '1.6rem', opacity: .55 }}>🎉</span>
            ...
            ? (
              <Card title="Nearby Classes" dotColor="#0ea5e9" to="/nearby-classes" linkLabel="All" isDark={isDark}>
                {nearbyClasses.map(cls => (
                  <div key={cls.entry_id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '9px 0', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,.04)' : 'rgba(0,0,0,.04)'}` }}>
              ...
              <Card title="Campus Overview" dotColor="#8b5cf6" isDark={isDark}>
                {isLeader && governanceStats
                  ? [
                    ...
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0', borderBottom: i < 2 ? `1px solid ${isDark ? 'rgba(255,255,255,.04)' : 'rgba(0,0,0,.04)'}` : 'none' }}>
                  ...
                  : (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '28px 16px', color: isDark ? '#94a3b8' : '#9ca3af', gap: 8 }}>
                      <span style={{ fontSize: '1.4rem', opacity: .45 }}>📍</span>
            ...
            {loadingBlog
              ...
              : blogPosts?.length
                ? blogPosts.slice(0, 4).map((post, i) => (
                  <Link key={post.id} to={`/blog/${post.slug}`} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '9px 0', borderBottom: i < Math.min(blogPosts.length, 4) - 1 ? `1px solid ${isDark ? 'rgba(255,255,255,.04)' : 'rgba(0,0,0,.04)'}` : 'none', textDecoration: 'none', color: 'inherit' }}>
                ...
                : (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '24px 16px', color: isDark ? '#94a3b8' : '#9ca3af', gap: 6 }}>
                    <span style={{ fontSize: '1.3rem', opacity: .45 }}>📝</span>
