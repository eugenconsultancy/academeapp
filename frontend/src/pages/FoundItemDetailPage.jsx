...
export default function FoundItemDetailPage() {
    ...
    const fetchItem = async () => {
        ...
        try { const data = await foundItemsApi.getItem(id); setItem(data); }
    ...
    const handleSendTip = async () => {
        ...
        catch (err) { toast.error('Failed to send tip'); }
    ...
    return (
        ...
        .fid-badge-red { background: rgba(239,68,68,0.08); color: #dc2626; }
                ...
                <div className="fid-card">
                    {item.blurred_image_url && (
                        <div className="fid-img">
                            <img src={item.blurred_image_url} alt={item.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        </div>
                    )}
                    ...
                    <div className="fid-body">
                        ...
                        {!isOwner && !item.is_claimed && (
                            <div className="fid-actions">
                                {!ownershipVerified ? (
                                    ...
                                    <button onClick={handleClaim} className="fid-btn fid-btn-green" style={{ flex: 1 }}>
                                        ✅ Proceed to Claim
                                    </button>
                                )}
                                <button onClick={() => setShowReportModal(true)} className="fid-btn fid-btn-danger">
                                    <FiFlag size={14} />
                                </button>
                                <button onClick={handleCopyLink} className="fid-btn fid-btn-outline">
                        ...
                        {!isOwner && (
                            <div className="fid-tip-row">
                                ...
                                <button onClick={handleSendTip} disabled={sendingTip || !tipMessage.trim()} className="fid-btn fid-btn-primary">
                                    <FiSend size={14} />
                                </button>
                            </div>
                        )}
