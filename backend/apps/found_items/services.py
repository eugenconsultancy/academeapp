...
class EscrowService:
    ...
    def initiate_payment(claim):
        ...
        PaymentTransaction.objects.create(
            ...
            phone_number=phone,
            status='PENDING',   # ALWAYS PENDING, webhook sets SUCCESS
            response_data=response
        )
    ...
    def disburse_funds(claim):
        if claim.item.payment_transactions.filter(transaction_type='B2C_DISBURSEMENT', status='SUCCESS').exists():
        ...
        locator_share = total * Decimal(str(settings.ESCROW_FEE_PERCENTAGE / 100))
    ...
    def auto_confirm_claims():
        threshold = timezone.now() - timedelta(days=settings.ESCROW_AUTO_CONFIRM_DAYS)
        claims = Claim.objects.filter(status='payment_received', confirmed_at__isnull=True, created_at__lte=threshold)
        for claim in claims:
...
class ClaimService:
    ...
    def get_claim_status(claim):
        ...
        elif claim.status == 'security_verified':
            next_step = 'evidence'
        elif claim.status == 'evidence_submitted':
            next_step = 'payment' if claim.item.is_fee_required else 'confirm'
        elif claim.status == 'awaiting_payment':
            next_step = 'payment'
        elif claim.status == 'payment_received':
            next_step = 'confirm'
        elif claim.status in ('claimed', 'completed'):
            next_step = 'done'
        else:
            next_step = 'claim'  # restart
        return {
            "claim_id": str(claim.id),
            "status": claim.status,
            "next_step": next_step,
            "requires_security": bool(claim.item.security_question and claim.status == 'pending'),
            "requires_payment": claim.item.is_fee_required and not claim.payment_received,
            "security_question": claim.item.security_question if claim.status == 'pending' else None,
        }
