...
class UserRole(Enum):
    ...
    FACULTY_OFFICER: The crucial delegation role.
    - Assigned by Institution Admin
    - Manages Class Reps for their department/faculty
    - Prevents Institution Admin from being a bottleneck
    """
    ADMIN = "admin"
...
# Thresholds for automatic badge awarding
BADGE_THRESHOLDS = {
    BadgeType.LOGIN_BRONZE: 10,      # 10 logins
    BadgeType.LOGIN_SILVER: 50,      # 50 logins
    BadgeType.LOGIN_GOLD: 100,       # 100 logins
    BadgeType.HIGH_ENGAGER: 25,      # 25 likes given
    BadgeType.EARLY_ADOPTER: 1,      # First login within first week
    BadgeType.TOP_CONTRIBUTOR: 50,   # 50 contributions
    BadgeType.PERFECT_ATTENDANCE: 100,  # 100% attendance for a semester
    BadgeType.COMMUNITY_HELPER: 10,  # 10 tips sent in found items
}
...
class ItemStatus(Enum):
    """Status lifecycle for found items"""
    PROCESSING = "processing"       # Image being blurred, admin review pending
    ACTIVE = "active"               # Visible to students, available for claims
    PENDING_CLAIM = "pending_claim" # Claim submitted, verification in progress
    CLAIMED = "claimed"             # Ownership verified, awaiting handover
    HANDED_OVER = "handed_over"     # Item returned to owner
    ARCHIVED = "archived"           # PII scrubbed, stored for records
    BLUR_FAILED = "blur_failed"     # Image processing failed, needs manual review
    DISPUTED = "disputed"           # Claim is being disputed
...
class ClaimStatus(Enum):
    """Status lifecycle for claims on found items"""
    PENDING = "pending"
...
class TicketStatus(Enum):
    """Status lifecycle for support tickets"""
    OPEN = "open"
...
class TransactionType(Enum):
    ...
    STUDENT_PAYMENT = "student_payment"      # Student pays platform (60 KES)
    FINDER_PAYOUT = "finder_payout"          # Platform pays finder (30 KES)
    PLATFORM_FEE = "platform_fee"            # Platform retains (30 KES)
...
# ============================================
# SPLIT STATUSES (for M-Pesa 30/30 split)
# ============================================

class SplitStatus(Enum):
    """Status of the payment split process"""
    PAID_TO_PLATFORM_ONLY = "paid_to_platform_only"  # Student paid, finder not yet paid
    FINDER_PAID = "finder_paid"                       # Finder has been paid
    FULLY_SETTLED = "fully_settled"                   # All parties settled
...
class AuditAction:
    ...
    Used throughout the application to ensure consistent
    audit trail entries. Each action represents a specific
    governance or security event.
    """
    # Role lifecycle
...
class SyncMethod(Enum):
    """Methods for syncing attendance records"""
    ONLINE = "online"
    OFFLINE = "offline"
    QR_CODE = "qr_code"
    LOCATION = "location"
...
class DayOfWeek(Enum):
    """Days of the week for timetable entries"""
    MONDAY = 0
    TUESDAY = 1
    WEDNESDAY = 2
    THURSDAY = 3
...
class PlatformDefaults:
    ...
    # Found Items
    RECOVERY_FEE_AMOUNT = "100.00"  # KES
    PLATFORM_FEE_PERCENTAGE = 50     # 50% to platform
    FINDER_PAYOUT_PERCENTAGE = 50    # 50% to finder
    AUTO_CONFIRM_CLAIM_DAYS = 7      # Days before auto-confirming
    PII_SCRUB_DELAY_DAYS = 7         # Days after handover to scrub PII
    
    # Announcements
    DEFAULT_EXPIRY_DAYS = 21         # 3 weeks default
    MAX_EXPIRY_DAYS = 60             # 2 months max
    
    # Opportunities
    DEFAULT_OPPORTUNITY_EXPIRY_DAYS = 120  # 4 months
    ...
    # Online status
    ONLINE_STATUS_TTL_SECONDS = 120  # 2 minutes
    
    # Audit
    AUDIT_LOG_RETENTION_DAYS = 365   # 1 year in live DB
    
    # Cache
    FEED_CACHE_TIMEOUT_SECONDS = 3600  # 1 hour
    GEOCODE_CACHE_TIMEOUT_SECONDS = 2592000  # 30 days


# ============================================
# SCOPE TYPES FOR STUDENT ROLES
# ============================================

class ScopeType(Enum):
    """Types of scopes for student leadership roles"""
    CLASS = "class"
    DEPARTMENT = "department"
    FACULTY = "faculty"
...
class ImageProcessing:
    """Constants for image processing pipeline"""
    ...
    ADDRESS_REGION_THRESHOLD = 0.7  # Bottom 30% of image